...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string,SDL_Surface*>::iterator sptr = surfaces.begin();
  while(sptr != surfaces.end()) {
	SDL_FreeSurface(sptr->second);
	sptr++;
  }
  std::map<std::string,SDL_Texture*>::iterator tptr = textures.begin();
  while (tptr != textures.end()) {
	SDL_DestroyTexture(tptr->second);
	tptr++;
  }
  std::map<std::string,Image*>::iterator iptr = images.begin();
  while (iptr != images.end()) {
	std::cout << "deleting " << iptr->first << std::endl;
	delete iptr->second;
	iptr++;
  }

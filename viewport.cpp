...
void Viewport::draw() const {
  ...
  IoMod::getInstance().writeText("FPS: " + std::to_string(Clock::getInstance().getFps()),
			gdata.getXmlInt("fpscounter/loc/x"), gdata.getXmlInt("fpscounter/loc/y"));
  IoMod::getInstance().writeText("Sam Fowler", 15, 425);
}
